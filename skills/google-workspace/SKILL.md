...
metadata:
  ...
  skill-audit-context-requires: explicit_workspace_goal, target_resource
  skill-audit-context-writes: query_results, remote_resources_changed, verification_result
  skill-audit-confirmation: on-risk
compatibility: Requires network access and a configured, authenticated Google Workspace integration; the fallback path additionally requires the `gws` CLI.
---

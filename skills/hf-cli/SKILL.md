...
metadata:
  skill-audit-context-reads: user_goal, hub_resource, local_paths, auth_status
  skill-audit-context-requires: explicit_hub_goal, target_operation_or_resource
  skill-audit-context-writes: query_results, files_changed, remote_resources_changed, commands_run, verification_result
  skill-audit-confirmation: on-risk
compatibility: Requires the `hf` CLI and network access to the Hugging Face Hub; private or mutating operations require Hub authentication.
---

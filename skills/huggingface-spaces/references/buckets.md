...
The Space's `HF_TOKEN` secret needs write permission on the bucket. Set it via
Settings → Secrets in the Space UI so the value does not appear in a command or
shell history.
